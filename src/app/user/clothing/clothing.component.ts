...
export class ClothingComponent implements OnInit {
  ...
  public filtredClothing = this.clothingList 
  ...
  filtredByPrice(p1: number,p2 : number) : void {
    this.filtredClothing = [];
    for (let i = 0; i < this.clothingList.length; i++) {
      if(this.clothingList[i].price > p1 && this.clothingList[i].price < p2){
        this.filtredClothing.push(this.clothingList[i]);
      }
    }
  }

  filtredByWishList(answer : boolean){
    if(answer == true){
      // this.filtredClothing = [];
      // this.clothingList.forEach(item => {
        // if(item.wishlist){
        //   this.filtredClothing.push(item);
        // }
        this.filtredClothing = this.selectedItems;

      // })
    }
    else{
      this.filtredClothing = this.clothingList;
    }
